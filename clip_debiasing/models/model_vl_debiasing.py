...
class DebiasedCLIP(nn.Module):
    ...
    def __init__(self, arch_str, device, mlp1_hidden_size=None, mlp2_hidden_size=None, alpha=None, debiasing_modules=True, **_kwargs,):
        ...
        # Load the standard pre-trained CLIP model
        self.clip, self.preprocess = clip.load(arch_str, device=device)
        
        # Freeze the original CLIP parameters to not destroy the pre-trained knowledge
        ...
        if debiasing_modules:
            ...
            # Initialize the "Debiasing" networks (MLPs), the only parts actually trained
            # mlp1 projects image embeddings, mlp2 projects text embeddings.
            ...
            # Enable gradient calculation for these new MLPs
            for param in list(self.mlp1.parameters()) + list(self.mlp2.parameters()):
                param.requires_grad = True 

            # Momentum Distillation (MoD), keep a queue of past features to stabilize training (like MoCo)
            ...
            # Create "Momentum" copies of the MLPs (Teacher networks)
            # These are updated slowly (exponential moving average) rather than by gradient descent directly
    ...
    def encode_text(self, text):
        ...
        # Apply the learned debiasing projection (residual connection: original + correction)
    ...
    def encode_image(self, image):
        ...
        # Apply the learned debiasing projection (residual connection), image side has two MLPs (mlp1 then mlp2)
    ...
    def forward(self, image, text1, text2, epoch):        
        with torch.no_grad():
            ...
            # 1. Get Original (Biased) Features from frozen CLIP
            ...
            # 2. Get Momentum (Teacher) Features (stabilizes training)
            # These provide stable targets for the student network to learn from
            image_embeddings = self.clip.encode_image(image)
            ...
            # Retrieve the queue of past samples (negatives)
            ...
            # Calculate similarity targets (Soft Labels)
            # How similar are current images to ALL texts (current batch + queue)?
        ...
        # Calculate "Fair" Features (Student Output) - what we are training!
        fair_image_features = F.normalize(self.encode_image(image), dim=-1)

        # text1 and text2 are likely counterfactual pairs (e.g., "photo of a man" vs "photo of a woman")
            ##why "likely", how do we know 
        fair_text1_features = F.normalize(self.encode_text(text1), dim=-1)
        fair_text2_features = F.normalize(self.encode_text(text2), dim=-1)

        # Calculate Bias Alignment Loss (Loss_BA)
        # We isolate the "bias" vector by subtracting the fair feature from the original feature.
        ...
        # We want the distribution of bias in images (e.g., gender cues) to match the distribution of bias in text.
        # This prevents the model from relying on spurious correlations that don't exist in the text.
        ...
        # Calculate Counterfactual Distillation Loss (Loss_CD)
        # Randomly pick one of the counterfactual texts (e.g., randomly pick "man" or "woman" text for a generic image)
        fair_text_random = fair_text1_features if random.random() < 0.5 else fair_text2_features

        # Calculate student similarities
        ...
        # Distillation: Force student similarities to match the mixed targets calculated earlier
        ...
        # Update the queue for next iteration
        self._dequeue_and_enqueue(original_image_features, original_text_features)
        
        # Total Loss: Weighted sum of Counterfactual Distillation and Bias Alignment
        return loss_cd * self.alpha + loss_ba * (1 - self.alpha)


    @torch.no_grad()
    def _dequeue_and_enqueue(self, image_feats, text_feats):
        # Update the queue of past features (used for negative samples in contrastive learning)
        # This is a standard MoCo (Momentum Contrast) technique to handle large batches
        batch_size = image_feats.shape[0]

        ptr = int(self.queue_ptr)
        # Replace the oldest features in the queue with the new batch
    ...
    def _momentum_update(self):
        # Update the "Teacher" (momentum) networks
        # Instead of training them directly, we slowly copy weights from the student (self.mlp)
        # formula: param_m = momentum * param_m + (1 - momentum) * param
    ...
    def copy_params(self):
        # Initialize the teacher networks to be identical to the student networks at start
        for model_pair in self.model_pairs:           
            for param, param_m in zip(model_pair[0].parameters(), model_pair[1].parameters()):
